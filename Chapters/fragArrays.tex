\chapter{Fragmented Array Elements}
%\authortoc{James G. Maloney}
%\chapterauthor{James G. Maloney}

\section{Direct Element Design}


\begin{itemize}
\item{direct element design }
\item{use large scan paper to describe method for including scan ( IEEE conf paper and slides )}
\item{example of whole x-band for the IEEE paper}
\item{4:1 bandwidth}
\item{discuss similarity to current sheets of munk and Harris}
\item{usually balanced fed with differential feeds and electronics}
\end{itemize}


\section{First Success}
\label{sec:ch6first}	% specifying a section label to be used by a \ref
\cite{Maloney1}.

\begin{figure}
\includegraphics[angle=0,width=\linewidth]{/Users/jim.maloney/Book/images/ch2/origPatentFig3.png}
\caption{Comparison of fragmented design to uniform aperture limit $2\pi A/\lambda^2$}
\label{fig:FAE1}		% figure references have to be below the \caption line
\end{figure}
