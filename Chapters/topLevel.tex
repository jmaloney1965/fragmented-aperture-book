% Fix Adobe Acrobat compatibility - force PDF 1.7 (pdftex only; xelatex emits >=1.5 unconditionally)
...
\usepackage{url} % for \url command in App B
\usepackage{booktabs} % \toprule \midrule \bottomrule in App B tables
\usepackage{float} % [H] float placement in App B figures
...
\usepackage{tocloft}
\setlength{\cftfignumwidth}{3.2em}  % widen figure number column in List of Figures
\setlength{\cfttabnumwidth}{3.2em}  % widen table number column in List of Tables
\setlength{\cftsecnumwidth}{3em}    % widen section number column in TOC
...
\include{singleElemFragmented}		% ch4	Advanced Fragmented Aperture Antennas
\include{reconfigurableFragmented}		% ch5
\include{WideBandArrays}				% ch6
\include{metaMaterialsAndAntennas}	% ch7
\include{reconfigurableArrays}			% ch8	show improved scan loss, dynamic polarization, etc.
\include{recentInnovations}			% ch9	level set methods, UAV swarm antennas, ML/AI
\include{otherEMStructures}			% ch10	combiners, metamaterials, phase surfaces

\appendix
\include{modelingAntennas}			% Appendix A — for Zander
\include{appendixB_classicalOptimization}  % Appendix B — classical optimization
