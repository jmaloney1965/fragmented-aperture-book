...
\begin{figure}
\begin{center}
\includegraphics[angle=-90,width=0.85\linewidth]{/Users/jim.maloney/Book/images/ch7/5996889-fig-3-hires.pdf}
\caption[Traditional fragmented aperture array construction]{Traditional fragmented aperture array construction.  Fragmented layers are printed on circuit board material and separated by foam spacers above a machined aluminum ground plane.  Feed towers enclose differential coaxial lines that connect to the dual-polarized elements \cite{MaloneyWideScan}.}
\label{fig:WSA2}
\end{center}
\end{figure}

This construction method can produce lightweight structures; for example, a 0.6~m $\times$ 0.6~m array built in this manner weighed only 4~kg for the entire antenna assembly.  This basic construction has been used to build many successful wideband fragmented aperture designs over the past two decades, including the 33:1 bandwidth array and the various designs summarized in the thickness-versus-frequency chart in Chapter~6 (Figure~\ref{fig:WBA22}).
...
The design process for fragmented aperture array elements uses a genetic algorithm to optimize a single unit cell of the array, terminated by periodic boundary conditions (PBC) that simulate an infinite array environment (see ``FDTD Analysis of Periodic Structures'' in Appendix~A for the FDTD formulation of PBC).  The FDTD method is ideal for this purpose because a single time-domain simulation produces the full frequency-domain response across the entire design bandwidth.

In the standard implementation, the PBC is applied as a wrap-around boundary at the edges of the unit cell, naturally modeling the infinite array at broadside.  This is the approach that was used to design the earlier fragmented aperture arrays, including the 33:1 bandwidth design.  However, standard broadside-only PBC does not provide information about off-broadside scan performance.  The scan volume narrowing observed in the 33:1 array (Figure~\ref{fig:WBA21}) was a direct consequence: the genetic algorithm had no mechanism to prevent scan volume degradation because it received no information about off-broadside behavior.

\subsection{The Spectral-Domain FDTD Approach}

To incorporate scan performance into the design process, a spectral-domain FDTD approach to periodic boundary conditions \cite{Aminian} was integrated into the design suite (see also ``Spectral-Domain FDTD'' in Appendix~A).  In the spectral-domain formulation, the FDTD simulation is performed at a fixed transverse wavenumber $(k_x, k_y)$ rather than at a fixed scan angle $(\theta, \phi)$.  The PBC is still implemented as a wrap-around boundary, but the constant transverse wavenumber assumption means that the effective scan angle is frequency dependent.
...
\begin{figure}
\begin{center}
\includegraphics[angle=0,width=0.85\linewidth]{/Users/jim.maloney/Book/images/ch7/5996889-fig-6-hires.pdf}
\caption[X-band element broadside realized gain]{Broadside embedded element realized gain for the whole X-band fragmented array element (blue line) compared to the uniform aperture area limit (red dots).  The realized gain is within approximately 0.2~dB of the theoretical limit across the 8--12~GHz design band \cite{MaloneyWideScan}.}
\label{fig:WSA5}
\end{center}
\end{figure}
...
\begin{figure}
\begin{center}
\includegraphics[angle=-90,width=0.85\linewidth]{/Users/jim.maloney/Book/images/ch7/5996889-fig-7-hires.pdf}
\caption[X-band element VSWR comparison]{VSWR comparison for the X-band fragmented array element.  The embedded element VSWR (blue line) and the broadside infinite-array scan VSWR (green line) are both below 2:1 across the design band.  The difference between the two reflects the influence of mutual coupling from neighboring elements \cite{MaloneyWideScan}.}
\label{fig:WSA6}
\end{center}
\end{figure}
...
\begin{figure}
\begin{center}
\includegraphics[angle=-90,width=0.85\linewidth]{/Users/jim.maloney/Book/images/ch7/5996889-fig-8-hires.pdf}
\caption[Embedded element gain pattern at 8~GHz]{Embedded element realized gain at 8~GHz as a function of azimuth and elevation angle for the V-pol feed.  The wide scan volume is evident, with usable gain extending well beyond $60^{\circ}$ in both planes \cite{MaloneyWideScan}.}
\label{fig:WSA7}
\end{center}
\end{figure}
...
\begin{figure}
\begin{center}
\includegraphics[angle=-90,width=0.85\linewidth]{/Users/jim.maloney/Book/images/ch7/5996889-fig-9-hires.pdf}
\caption[Embedded element gain pattern at 10~GHz]{Embedded element realized gain at 10~GHz.  The wide scan volume ($> \pm 60^{\circ}$) is maintained at the center of the X-band \cite{MaloneyWideScan}.}
\label{fig:WSA8}
\end{center}
\end{figure}
...
\begin{figure}
\begin{center}
\includegraphics[angle=-90,width=0.85\linewidth]{/Users/jim.maloney/Book/images/ch7/5996889-fig-10-hires.pdf}
\caption[Embedded element gain pattern at 12~GHz]{Embedded element realized gain at 12~GHz.  Some slight reduction in the scan volume is visible in the azimuth direction, but the overall scan volume still substantially exceeds $\pm 60^{\circ}$ \cite{MaloneyWideScan}.}
\label{fig:WSA9}
\end{center}
\end{figure}
...
\textcolor{red}{[TODO: Include a thickness vs.\ frequency chart for PCB designs analogous to the air-filled cavity chart in Chapter~6, once enough PCB designs have been completed to establish the trend.]}

\subsection{Independent Validation by Other Research Groups}

The wide-scan fragmented aperture array concept has been independently investigated by researchers outside of GTRI.  Ellgardt and Persson at the Royal Institute of Technology (KTH) in Sweden \cite{Ellgardt2006Wide} designed and analyzed wide-scan fragmented aperture elements for operation in the 1.5--4.5~GHz range with scan volumes up to $60^{\circ}$.  Their work addressed two practical challenges: restricting dielectric materials to commercially available substrates (rather than allowing arbitrary permittivity values during optimization), and resolving the diagonal fragment contact problem discussed in Chapter~3.

Figure~\ref{fig:EllgardtNoSup} shows a design without a superstrate, using a $20 \times 20$ fragment grid on a substrate with $\epsilon_r = 2.2$ and thickness $d_1 = 10$~mm.  This element achieves VSWR~$\leq 2$ over a 1.94:1 bandwidth for $45^{\circ}$ scan in all planes, but does not meet the VSWR~$\leq 2$ criterion for $60^{\circ}$ scan.  The H-plane is consistently the limiting factor for wide-scan performance.

\begin{figure}
\begin{center}
\includegraphics[angle=0,width=\linewidth]{/Users/jim.maloney/Book/images/ch7/ellgardt2006/Ellgardt2006_Fig6_NoSuperstrate.png}
\caption[Ellgardt wide-scan element without superstrate]{Wide-scan fragmented array element without superstrate: (a)~reflection coefficient at broadside and E/H-plane scan to $45^{\circ}$ and $60^{\circ}$; (b)~$20 \times 20$ fragment unit cell geometry ($d_1 = 10$~mm, $\epsilon_{r1} = 2.2$, $Z_c = 171~\Omega$).  The element achieves VSWR~$\leq 2$ over 1.94:1 bandwidth for $45^{\circ}$ scan \cite{Ellgardt2006Wide}.}
\label{fig:EllgardtNoSup}
\end{center}
\end{figure}

Figure~\ref{fig:EllgardtSup} shows a second design that adds a dielectric superstrate ($d_2 = 8$~mm, $\epsilon_{r2} = 3.38$) above the fragmented layer.  The superstrate improves the scan bandwidth to 2.20:1 for $45^{\circ}$ scan and 2.16:1 for $60^{\circ}$ scan (at VSWR~$\leq 3$), though it introduces a surface wave resonance near 4~GHz.  These results---obtained completely independently using the same GA/FDTD design methodology---corroborate the scan performance achievable with fragmented aperture elements and highlight the role of superstrate layers in extending the scan volume.

\begin{figure}
\begin{center}
\includegraphics[angle=0,width=\linewidth]{/Users/jim.maloney/Book/images/ch7/ellgardt2006/Ellgardt2006_Fig7_WithSuperstrate.png}
\caption[Ellgardt wide-scan element with superstrate]{Wide-scan fragmented array element with superstrate: (a)~reflection coefficient at multiple scan angles; (b)~unit cell geometry ($d_1 = 8.67$~mm, $\epsilon_{r1} = 3.00$, $d_2 = 8$~mm, $\epsilon_{r2} = 3.38$, $Z_c = 173~\Omega$).  The superstrate improves scan bandwidth to 2.20:1 for $45^{\circ}$ scan \cite{Ellgardt2006Wide}.}
\label{fig:EllgardtSup}
\end{center}
\end{figure}
...
\bibitem{Ellgardt2006Wide} A.~Ellgardt and P.~Persson, ``Characteristics of a broad-band wide-scan fragmented aperture phased array antenna,'' in Proc.\ European Conference on Antennas and Propagation (EuCAP), Nice, France, November 2006, ESA SP-626.
