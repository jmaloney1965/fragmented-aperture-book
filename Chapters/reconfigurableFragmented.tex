...
The flat Agile Aperture Antenna described above provides beam steering within the half-space in front of the aperture.  Kiesel and Strates \cite{Kiesel2017Cylindrical} at GTRI extended the switched-pixel concept to a cylindrical geometry to achieve full hemispherical coverage from a single antenna element with no phased array beamformer.

The cylindrical antenna is constructed from modular ``smart pixel'' boards---the same COTS pixel technology developed for the flat reconfigurable aperture.  Each board contains four RF switches, a control circuit, and bias/RF bus connections, costing approximately US\$10 per pixel with 300~$\mu$W DC consumption.  Two antenna sizes were built: a 133-mm diameter cylinder (24 pixels, 140~mm tall) and a 75-mm diameter cylinder (24 body pixels + 5 top-cap pixels, 85~mm tall).  Figure~\ref{fig:RECAP_CYL1} shows the 133-mm cylindrical antenna.
...
\caption[Cylindrical reconfigurable pixel antenna]{The 133-mm diameter cylindrical reconfigurable pixel antenna mounted on a ground plane in the anechoic chamber.  The smart pixel boards wrap around the cylinder, providing 24 independently switchable pixels for full azimuthal beam steering \cite{Kiesel2017Cylindrical}.}
